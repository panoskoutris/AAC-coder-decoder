"""
Demo for AAC Level 2 Encoder and Decoder
Demonstrates the complete encode-decode pipeline with TNS and calculates SNR.
"""
...
def demo_aac_2(filename_in, filename_out):
    """
    Demonstrates Level 2 AAC encoding and decoding with TNS.
    
    Takes an input WAV file, encodes it using aac_coder_2 (with TNS), decodes 
    it using i_aac_coder_2, writes the decoded audio to an output file, and 
    calculates the Signal-to-Noise Ratio (SNR) between the original and decoded audio.
    ...
    print(f"  Encoded into {len(aac_seq_2)} frames")
    
    # Decode the audio
    print("\nDecoding with inverse TNS...")
    decoded_audio = i_aac_coder_2(aac_seq_2, filename_out)
